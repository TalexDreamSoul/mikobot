...
## Highlights
- **2026-09-21** Released **v0.7.4** — session-history authorization no longer reads what it forbids: search decides access from session metadata instead of parsing a foreign transcript, and filters candidates before reading any message body. `my` gains a read-only `channels` check that shows a member turn exactly its own channel instance (four fields, no owners or errors), and scoped turns are told plainly where a relative path lands. Please see [release notes](https://github.com/TalexDreamSoul/mikobot/releases/tag/v0.7.4) for details.
